...
function isLikelyUnlabeledYear(match, nodeValue, index) {
    ...
    // Alphanumeric designator + hyphen prefix (B-17, VMSB-241, SB2U-3, I-495, A-320, etc.)
    // Only block when the char before the hyphen is uppercase or a digit — preserves
    // temporal prefix words like post-, pre-, mid- (which end in lowercase letters).
    if (/[A-Z0-9]-$/.test(rawBefore)) return false;
    ...
    const precedingInhibitors = new Set([
        ...
        'flight', 'mission',
        // Military unit designators (defense-in-depth for ≥1000 unit numbers)
        'squadron', 'regiment', 'battalion', 'brigade', 'division',
        'company', 'platoon', 'fleet', 'corps', 'wing', 'group', 'battery',
    ...
    const units = [
        ...
        // Medical
        "mcg", "iu",
        // Military / casualty abbreviations
        "kia", "wia", "mia",
    ...
    //   acceptable approximation; labelled occurrences are handled by the era processor.
    const famousYearContexts = new Map([
        // Sub-100 CE
        [33,  ['crucifixion', 'jesus', 'christ', 'calvary', 'golgotha', 'resurrection']],
        [79,  ['vesuvius', 'pompeii', 'herculaneum', 'eruption', 'pliny']],
        // Sub-100 — historically BCE, converts as CE when unlabeled
        [44,  ['julius caesar', 'ides', 'brutus', 'calpurnia', 'assassination of caesar']],
        // 100–999
        [117, ['trajan', 'hadrian', 'dacian', 'parthia', 'roman empire']],
        [180, ['marcus aurelius', 'aurelius', 'antonine', 'stoic', 'meditations', 'commodus']],
        [212, ['caracalla', 'constitutio antoniniana', 'roman citizenship']],
        [284, ['diocletian', 'tetrarchy', 'reform']],
        [312, ['constantine', 'milvian', 'edict of milan', 'licinius']],
        [325, ['nicaea', 'council of nicaea', 'constantine', 'creed']],
        [380, ['theodosius', 'edict of thessalonica', 'christianity']],
        [410, ['visigoths', 'alaric', 'sack of rome', 'rome sacked']],
        [451, ['catalaunian', 'attila', 'hun', 'aetius']],
        [476, ['romulus augustulus', 'odoacer', 'fall of rome', 'western roman empire', 'fall of the western']],
        [480, ['thermopylae', 'salamis', 'leonidas', 'xerxes', 'spartan']],
        [490, ['marathon', 'miltiades', 'darius', 'athenian']],
        [529, ['justinian', 'platonic academy', 'corpus juris', 'justinian code']],
        [570, ['muhammad', 'mohammed', 'mohammad', 'mohamed', 'mahomet', 'muhammed',
               'mecca', 'prophet of islam']],
        [622, ['hijra', 'hijrah', 'hegira', 'muhammad', 'medina', 'islamic calendar']],
        [711, ['moorish', 'moors', 'iberia', 'tariq', 'umayyad', 'caliphate']],
        [732, ['tours', 'poitiers', 'charles martel', 'martel']],
        [793, ['lindisfarne', 'viking', 'norse', 'northumbria']],
        [800, ['charlemagne', 'holy roman empire', 'pope leo', 'aachen', 'coronation']],
        [960, ['song dynasty', 'zhao kuangyin']],
        [987, ['capetian', 'hugh capet', 'capet']],
        // 1000+ — converts by default already; listed for future threshold-raise resilience
        [1066, ['hastings', 'william the conqueror', 'normans', 'harold', 'domesday']],
    ]);
    ...
    return true;  // year >= 1000 → default to convert
}
...
function processPluralReferences(text) {
    ...
    return text.replace(pluralRegex, (match, fuzzyPrefix, numberStr, eraStr, offset, fullText) => {
        ...
        // Don't convert alphanumeric designation numbers (B-17s, SB2U-3s, VMSB-241s)
        const charBeforeMatch = fullText[offset - 1];
        const charBeforeThat  = offset >= 2 ? fullText[offset - 2] : '';
        if (charBeforeMatch === '-' && /[A-Za-z0-9]/.test(charBeforeThat)) return match;
...
const allTests = [
    ...
    { input: "300 is thought to be the fall of Rome", expected: "300 is thought to be the fall of Rome" },
    ...
    { input: "233 carrier-based aircraft", 
        expected: "233 carrier-based aircraft" },
    { input: "127 land-based aircraft", 
    ...
    { input: "In addition, many of the aircraft being used during the June 1942 operations had been operational since late November 1941 and, although they were well-maintained, many were almost worn out and had become increasingly unreliable.", 
        expected: "In addition, many of the aircraft being used during the June 11942 H.E. (Holocene Era) [converted from 1942 CE] operations had been operational since late November 11941 H.E. (Holocene Era) [converted from 1941 CE] and, although they were well-maintained, many were almost worn out and had become increasingly unreliable." },
    { input: "Nine B-17s took off from Midway at 12:30 for the first air attack.", 
        expected: "Nine B-17s took off from Midway at 12:30 for the first air attack." },
    { input: "Of the 108 Japanese aircraft that participated in this attack, 11 were destroyed (including 3 that ditched), 14 were heavily damaged, and 29 were damaged to some degree. 140 more were available to the Japanese, but never launched, and were ultimately destroyed when their carriers sank.", 
        expected: "Of the 108 Japanese aircraft that participated in this attack, 11 were destroyed (including 3 that ditched), 14 were heavily damaged, and 29 were damaged to some degree. 140 more were available to the Japanese, but never launched, and were ultimately destroyed when their carriers sank." },
    { input: "Having taken off prior to the Japanese attack, American bombers based on Midway made several attacks on the Japanese carrier force. These included six Grumman Avengers, detached to Midway from Hornet's VT-8 (Midway being the combat debut of both VT-8 and the Avenger); Marine Scout-Bombing Squadron 241 (VMSB-241), consisting of 11 SB2U-3s and 16 SBDs, plus four USAAF B-26s of the 18th Reconnaissance and 69th Bomb Squadrons armed with torpedoes, and 15 B-17s of the 31st, 72nd, and 431st Bomb Squadrons. The Japanese repelled these attacks, losing only three Zero fighters while destroying five Avengers, two SB2Us, eight SBDs, and two B-26s.[72][73] Among the dead was Major Lofton R. Henderson of VMSB-241, killed while leading his inexperienced SBD squadron into action. (The main airfield at Guadalcanal was named after him in August 1942.)", 
        expected: "Having taken off prior to the Japanese attack, American bombers based on Midway made several attacks on the Japanese carrier force. These included six Grumman Avengers, detached to Midway from Hornet's VT-8 (Midway being the combat debut of both VT-8 and the Avenger); Marine Scout-Bombing Squadron 241 (VMSB-241), consisting of 11 SB2U-3s and 16 SBDs, plus four USAAF B-26s of the 18th Reconnaissance and 69th Bomb Squadrons armed with torpedoes, and 15 B-17s of the 31st, 72nd, and 431st Bomb Squadrons. The Japanese repelled these attacks, losing only three Zero fighters while destroying five Avengers, two SB2Us, eight SBDs, and two B-26s.[72][73] Among the dead was Major Lofton R. Henderson of VMSB-241, killed while leading his inexperienced SBD squadron into action. (The main airfield at Guadalcanal was named after him in August 11942 H.E. (Holocene Era) [converted from 1942 CE].)" },
    { input: "While the Japanese were able to launch 108 aircraft in just seven minutes, it took Enterprise and Hornet over an hour to launch 117.[102] Spruance judged that the need to throw something at the Japanese as soon as possible was greater than the coordination the attack by aircraft of different types and speeds (fighters, bombers, and torpedo bombers).", 
        expected: "While the Japanese were able to launch 108 aircraft in just seven minutes, it took Enterprise and Hornet over an hour to launch 117.[102] Spruance judged that the need to throw something at the Japanese as soon as possible was greater than the coordination the attack by aircraft of different types and speeds (fighters, bombers, and torpedo bombers)." },
